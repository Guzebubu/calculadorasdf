...
@app.route('/lv1')
def lv1_page(): return render_template('lv1.html')

@app.route('/lv2')
def lv2_page(): return render_template('lv2.html')

@app.route('/medios')
def medios_page(): return render_template('medios.html')

@app.route('/molaridad')
def molaridad_page(): return render_template('molaridad.html')

@app.route('/calculos_generales')
def calculos_generales_page(): return render_template('calculos_generales.html')

@app.route('/recetas')
def recetas_page(): return render_template('recetas.html')
...
def calcular_pcr_master():
    try:
        ...
        for pcr in pcrs_input:
            ...
            nombre_pcr = pcr.get('nombre', 'PCR')
            num_reacciones = float(pcr.get('num_reacciones', 8))
            porcentaje_extra = float(pcr.get('porcentaje_extra', 10))
            vol_total = float(pcr.get('vol_total', 10.0))
            vol_adn = float(pcr.get('vol_adn', 1.0))
            factor_extra = 1 + (porcentaje_extra / 100)
            
            resultados = {'nombre': nombre_pcr}

            if tipo_pcr == 'inicial':
                base_buffer = vol_total * 0.20
                base_fw = vol_total * 0.04
                base_rv = vol_total * 0.04
                base_polimerasa = vol_total * 0.008
                base_agua = vol_total - (base_buffer + base_fw + base_rv + base_polimerasa + vol_adn)
                ...
                base_buffer = vol_total * 0.10
                base_dntps = vol_total * 0.02
                base_fw = vol_total * 0.04
                base_rv = vol_total * 0.04
                base_polimerasa = vol_total * 0.008
                base_agua = vol_total - (base_buffer + base_dntps + base_fw + base_rv + base_polimerasa + vol_adn)
                ...
                base_buffer = float(pcr.get('base_buffer', 1.0))
                base_mgcl2 = float(pcr.get('base_mgcl2', 1.0))
                base_dntps = float(pcr.get('base_dntps', 0.2))
                base_fw = float(pcr.get('base_fw', 0.4))
                base_rv = float(pcr.get('base_rv', 0.4))
                base_polimerasa = float(pcr.get('base_polimerasa', 0.5))
                base_agua = vol_total - (base_buffer + base_mgcl2 + base_dntps + base_fw + base_rv + base_polimerasa + vol_adn)
